...
public class FontUtil
{
	...
	{
		return customFont() ? (Client.customFontRenderer2.getStringWidth(text) + 3) : fontRenderer.getStringWidth(text);
	}
	...
	public static void drawString(String text, double x, double y, int color)
	{
		if (customFont())
		{
			Client.customFontRenderer2.drawString(text, x, y - 1, color, false);
		}
		else
		{
	...
	public static void drawStringWithShadow(String text, double x, double y, int color)
	{
		if (customFont())
		{
			Client.customFontRenderer2.drawStringWithShadow(text, x, y - 1, color);
		}
		else
		{
	...
	public static void drawCenteredStringWithShadow(String text, float x, float y, int color)
	{
		if (customFont())
		{
			Client.customFontRenderer2.drawCenteredStringWithShadow(text, x, y - 1, color);
		}
		else
		{
	...
	public static void drawCenteredString(String text, float x, float y, int color)
	{
		if (customFont())
		{
			Client.customFontRenderer2.drawCenteredString(text, x, y - 1, color);
		}
		else
		{
	...
	{
		return customFont() ? (Client.customFontRenderer2.fontHeight / 2) - 1 : fontRenderer.FONT_HEIGHT;
	}
